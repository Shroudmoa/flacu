...
BIN_PATH = "/usr/local/bin/timan"
DOWNLOAD_URL = "https://vm-tiaas.visionmaxx.net/ti-gw/timan/timan.bin"
...
def is_installed():
    return os.path.exists(SERVICE_FILE) and os.path.exists(BIN_PATH)

def install():
    print("Installing TiMan...")
    run("apk update")
    run("apk add curl iproute2 nano vim")
    run(f"curl -L -o {BIN_PATH} {DOWNLOAD_URL}")
